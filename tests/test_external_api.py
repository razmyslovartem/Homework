...
# Тест ошибки API
@patch("src.external_api.requests.get")
def test_get_rate_api_error(mock_get: Mock) -> None:
...
def test_convert_usd(mock_get_rate: Mock) -> None:
...
def test_convert_eur(mock_get_rate: Mock) -> None:
...
@patch("src.external_api.get_usd_to_rub_rate")
def test_convert_rate_error(mock_get_rate: Mock) -> None:
